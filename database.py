...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        # Принудительно добавляем колонки, если их нет
        try:
            await conn.execute(
                text(
                    "ALTER TABLE channels ADD COLUMN IF NOT EXISTS ai_model VARCHAR DEFAULT 'groq'"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE channels ADD COLUMN IF NOT EXISTS custom_prompt TEXT"
                )
            )
...
async def set_active_channel(user_id: int, channel_id: int):
    async with async_session() as session:
        await session.execute(
            ...
            .values(active_channel_id=channel_id)
        )
        await session.commit()
...
async def update_channel_ai_model(channel_id: int, ai_model: str):
    async with async_session() as session:
        await session.execute(
            update(Channel).where(Channel.id == channel_id).values(ai_model=ai_model)
        )
        await session.commit()

async def update_channel_prompt(channel_id: int, prompt: str):
    async with async_session() as session:
        await session.execute(
            update(Channel).where(Channel.id == channel_id).values(custom_prompt=prompt)
        )
        await session.commit()
...
async def get_active_sources(channel_id: int) -> list[Source]:
    async with async_session() as session:
        result = await session.execute(
            select(Source).where(
                Source.channel_id == channel_id, Source.enabled.is_(True)
            )
        )
        return list(result.scalars().all())
...
async def get_channel_drafts(
    channel_id: int, status: str = "pending", limit: int = 10
) -> list[Draft]:
    async with async_session() as session:
        result = await session.execute(
            select(Draft)
...
async def update_draft_text(draft_id: int, text: str):
    async with async_session() as session:
        await session.execute(
            update(Draft).where(Draft.id == draft_id).values(post_text=text)
        )
        await session.commit()
    ...
    async with async_session() as session:
        result = await session.execute(
            select(PublishedPost).where(
                PublishedPost.channel_id == channel_id, PublishedPost.source_url == url
            )
        )
...
async def get_channel_stats(channel_id: int) -> dict:
    async with async_session() as session:
        ...
        total_res = await session.execute(
            select(func.count(Draft.id)).where(Draft.channel_id == channel_id)
        )
...
async def schedule_post(channel_id: int, draft_id: int, scheduled_time: datetime):
    async with async_session() as session:
        sp = ScheduledPost(
            channel_id=channel_id, draft_id=draft_id, scheduled_time=scheduled_time
        )
        session.add(sp)
        await session.execute(
            update(Draft).where(Draft.id == draft_id).values(status="scheduled")
        )
        await session.commit()
...
async def mark_scheduled_published(scheduled_id: int):
    async with async_session() as session:
        ...
        if sp:
            sp.status = "published"
            await session.execute(
                update(Draft).where(Draft.id == sp.draft_id).values(status="published")
            )
            await session.commit()
