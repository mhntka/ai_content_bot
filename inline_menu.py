...
def get_channels_keyboard(
    channels: list, active_channel_id: int = None
    ...
    for ch in channels:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=f"{marker} {ch.name}", callback_data=f"set_channel_{ch.id}"
                )
            ]
        )
...
def get_draft_keyboard(draft_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Сейчас", callback_data=f"approve_{draft_id}"
                ),
                InlineKeyboardButton(
                    text="📅 Запланировать", callback_data=f"pre_schedule_{draft_id}"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="✏️ Редактировать", callback_data=f"edit_draft_{draft_id}"
                ),
                InlineKeyboardButton(
                    text="❌ Пропустить", callback_data=f"skip_{draft_id}"
                ),
            ],
        ]
    )
...
def get_schedule_keyboard(draft_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="+1 час", callback_data=f"do_schedule_{draft_id}_1"
                ),
                InlineKeyboardButton(
                    text="+3 часа", callback_data=f"do_schedule_{draft_id}_3"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="+6 часов", callback_data=f"do_schedule_{draft_id}_6"
                ),
                InlineKeyboardButton(
                    text="+12 часов", callback_data=f"do_schedule_{draft_id}_12"
                ),
            ],
...
def get_settings_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🤖 Выбрать ИИ", callback_data="select_ai_model"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📝 Кастомный промпт", callback_data="edit_prompt"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🗑 Удалить текущий канал", callback_data="delete_channel"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🔄 Сбросить источники", callback_data="reset_sources"
                )
            ],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="main_menu")],
        ]
...
def get_ai_model_keyboard(current_model: str) -> InlineKeyboardMarkup:
    ...
    for key, name in models.items():
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=f"{marker}{name}", callback_data=f"set_ai_{key}"
                )
            ]
        )

    keyboard.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="settings")])
